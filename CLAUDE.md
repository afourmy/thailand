...
Fixed so far (2026-07-10): wlt-c21-023 ex1_1 (ทารก‸ดูดนมแม่), tobo-208 ex0_1 (เลีย‸ไอศกรีม), tamago-l12-097 ex0_1 (อย่า‸งอนสิ) — ‸ marks where the invisible ZWSP sits. 2026-08-10: thaipod-1392, all three Thai clips (ใจ‸เย็นๆ), the ๆ-scope case; thaipod-1342 ex0_1 (ขากลับบ้าน·แวะ·หายายหน่อย), the isolation case, see the section below. Note the last one: whisper transcribed the bad clip as the expected string (อย่างอน is the same letters read either way), so only the differential test could detect it. 2026-08-14, all isolation cases: wlt-c09-076 ex0_1 (ดื่มเหล้า·แล้ว·อย่าขับรถ), wlt-c17-054 ex0_1 (เขาซื้อข้าว·และ·ไข่), wlt-c10-045 ex0_1 (ขอ·เข้า·นอนก่อนนะ), wlt-c19-094 ex0_0 (ขาฉัน·เมื่อย·มาก); plus two missing clause breaks fixed in the display `thai` itself, not in `thai_tts`: tamago-l3-284 ex0_1 (อย่ามากวน น่ารำคาญ) and wlt-c21-014 ex0_1 (ขับตรงไป อย่าเลี้ยว). When the space is genuinely missing from the Thai sentence, fix the sentence: `thai_tts` is for hints Azure needs that the reader does not. 2026-09-08: t4k-c11-020 สาธารณรัฐ, word clip plus both sentences, `สาธารณ‌รัฐ` with a ZWNJ; a compound-boundary case, see below.

**A dropped linking syllable inside one word is the same bug at a compound boundary.** สาธารณรัฐ is sǎa-thaa-rá-ná-rát: the ร of ...ารณ is read twice, once closing รา and once opening the next syllable. Azure read it "sǎa-thaa-ná-rát", dropping the ระ, because it parsed สาธารณ + รัฐ as one run. A separator at the compound boundary restores it. This affects the word clip and every sentence containing the word, so hint all of them and regenerate the card with both generators. Sibling risk: any ...ารณ + consonant compound (สาธารณสุข, สาธารณูปโภค) can drop the same syllable; test them when one turns up.
...
**Put the separators at the outer edges of the whole word, never inside it.** A compound is one lexical unit and splitting it is its own mispronunciation. พยาบาลเฝ้าคนไข้ตลอดทั้งคืน (tamago-l12-546 ex0_1) was hinted `คน·ไข้·ตลอด`, which isolated ไข้ and cut คนไข้ in half; the word that needed isolating was คนไข้, because Azure was swallowing it into เฝ้าคน. Identify the word first (its boundaries, not the syllable that sounds wrong), then wrap it: `เฝ้า‌คนไข้‌ตลอด`. Re-fixing this cost a second round of listening on 2026-09-08.
...
2. **ZWNJ (U+200C)** when the dot's break is audible, and now the first thing to try alongside the dot rather than a late fallback: it won twice on 2026-09-08, on สาธารณ‌รัฐ (word-internal, where the dot fixed the reading but the user heard the gap) and on พยาบาลเฝ้า‌คนไข้‌ตลอดทั้งคืน (in a sentence, chosen over both one and two dots). Probe it against ZWSP and the word joiner (U+2060) at the same boundary; all three synthesize to the same length, so only the ear separates them.
3. **Hyphen `-` on both sides.** Also read correctly, adds ~120ms, slightly more noticeable than the dot.
4. **Comma**, i.e. a real space in `thai_tts`, which `thai_body()` converts to ", ". Isolates most strongly and always fixes the reading, but the ~0.9s pause is plainly audible, so it is a diagnostic tool, not a shipping fix: use it first to confirm that isolation is what the word needs, then move down to the dot.
5. **ZWSP (U+200B)** only helps segmentation splits and ๆ scope. For a wrong word reading it frequently changes nothing at all (four ZWSP placements around แวะ produced clips of identical length to the broken one).
