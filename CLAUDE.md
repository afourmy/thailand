...
2. **ZWNJ (U+200C)** when the dot's break is audible. Inside a single word the dot's ~50ms is noticeable in a way it is not between words: on สาธารณรัฐ (2026-09-08) the dot fixed the reading but the user heard the gap. A zero-width non-joiner at the same boundary (สาธารณ‌รัฐ) gave the same reading with no gap. Probe it against ZWSP and the word joiner (U+2060) at the same boundary; all three synthesize to the same length, so only the ear separates them.
