...
**Which words:** we now add example sentences to *occasional* words in every topic, i.e. words whose `frequency` field in `vocab.json` is `"occasional"`. The `everyday`, `common`, and `rare` words are already done, so this is the last remaining category. Work through the matching words that don't yet have an `examples` field, ten at a time (see `example_sentences_done.md` for what's already done).
