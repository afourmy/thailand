...
**Which words:** we now add example sentences to *common* words in every topic *except* `general`, i.e. words whose `frequency` field in `vocab.json` is `"common"` **and** whose `topic` field is anything other than `"general"`. The `everyday` words and the `common` + `general` words are already done; skip the other frequencies (`occasional`, `rare`) for now. Work through the matching words that don't yet have an `examples` field, ten at a time (see `example_sentences_done.md` for what's already done).
...
- **Cover every distinct meaning of the word** - a word's meanings are separated by semicolons in the translation: "A, B, C; D, E" means the word has two meanings (one is *A, B, C*, the other is *D, E*). Give two example sentences per meaning, and make each one show something different (a different context, collocation, or grammatical role) rather than restating the same usage twice. **Two is the default; a single example is a rare exception, not something to reach for.** The one-example exception applies *mainly* to cards whose headword is itself a complete fixed expression or whole sentence (e.g. `นกเขาขันตอนเช้า`, "morning wood"): that whole clause can't be re-shown in a meaningfully different second context, so it gets a single example. **This exception must never be applied to an ordinary word, and never to a single verb.** A verb always varies by who does it, to whom, and in what situation, so two genuinely different sentences are always possible, even for a crude, taboo, or "single-act" verb (e.g. `โม๊ก`, `เอากัน`, `กินตับ`): write two. Only drop to one when, after genuinely trying, no second sentence would show anything new, which in practice means fixed full-sentence expressions and almost nothing else. Phrases that slot naturally into varied surrounding speech (e.g. `ขอคิดดูก่อน`, `จบข่าว`) also take two. When in doubt, write two.
...
2. **Stop *only* if there's something for me to decide.** If the sanity check skipped any card, report those (each with which meaning failed and which test: gloss mismatch, or can't be exemplified naturally) and **wait for my reply** before doing anything else, since I may clarify or correct a skipped card so it can be done. If nothing was skipped, do **not** stop and do **not** wait for confirmation: report the batch briefly and move straight on to the steps below.
