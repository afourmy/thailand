...
**Which words:** we now add example sentences to *rare* words in every topic, i.e. words whose `frequency` field in `vocab.json` is `"rare"`. The `everyday` and `common` words are already done; skip `occasional` for now. Work through the matching words that don't yet have an `examples` field, ten at a time (see `example_sentences_done.md` for what's already done).
