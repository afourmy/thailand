...
- **Cover every distinct meaning of the word** - a word's meanings are separated by semicolons in the translation: "A, B, C; D, E" means the word has two meanings (one is *A, B, C*, the other is *D, E*). Aim for two example sentences per meaning, and make each one show something different (a different context, collocation, or grammatical role) rather than restating the same usage twice. Two is a target, not padding: give a second sentence **only when it is genuinely different**. If a genuinely distinct second sentence can't be written, give just one rather than re-wrap the same usage. This especially applies to cards whose headword is itself a complete fixed expression or whole sentence (e.g. `นกเขาขันตอนเช้า`, "morning wood"): such a phrase can't be shown in a meaningfully different second context, so it gets a single example. Real words, and phrases that slot naturally into varied surrounding speech (e.g. `ขอคิดดูก่อน`, `จบข่าว`), can still take two distinct sentences, so keep two there.
