...
2. **Report and stop for my input.** As soon as the batch ends, report to me: (a) the result of the second pass, and (b) every card you skipped during the sanity check, each with which meaning failed and which test (gloss mismatch, or can't be exemplified naturally). Then **wait for my reply** before going further: I may clarify or correct a skipped card so it can be done. Do not run the steps below until I've responded.
